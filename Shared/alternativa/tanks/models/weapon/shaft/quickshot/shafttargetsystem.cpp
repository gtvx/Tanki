...
class ShaftMultybodyCollisionPredicate : public IRayCollisionPredicate
{
public:
	QSet<Body*> bodies;
	Body *shooterBody;
...
ShaftTargetSystem::ShaftTargetSystem()
{
	currHitPoints = new ListVector3;
	multybodyPredicate = new ShaftMultybodyCollisionPredicate;
}
...
void ShaftTargetSystem::setParams(ICollisionDetector *collisionDetector, double upAngle, int upRaysNum, double downAngle, int downRaysNum, double weakeningCoeff, ICTFModel *ctfModel)
{
...
void ShaftTargetSystem::getTargets(TankData *shooterData,
...
void ShaftTargetSystem::checkAllDirections(TankData *shooterData,
										   const Vector3 *barrelOrigin,
										   const Vector3 *baseDir,
										   const Vector3 *rotationAxis,
										   QHash<Body*, TankData*> *tanks,
...
void ShaftTargetSystem::checkDirection(const Vector3 *barrelOrigin,
...
double ShaftTargetSystem::evaluateDirection(const Vector3 *barrelOrigin,
											const Vector3 *barrelDir,
											TankData *shooterData,
											QHash<Body*, TankData*> *tanks,
...
void ShaftTargetSystem::collectTargetsAlongRay(TankData *shooterData,
											   const Vector3 *barrelOrigin,
											   const Vector3 *barrelDir,
											   QHash<Body*, TankData*> *tanks,
											   ShaftShotResult *shotResult)
...
double ShaftTargetSystem::getDirectionPriority(double angle, double distance)
{
