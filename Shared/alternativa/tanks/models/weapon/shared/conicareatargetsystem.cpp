#include "conicareatargetsystem.h"
...
class ConicAreaGunPredicate : public IRayCollisionPredicate
{
...
ConicAreaTargetSystem::ConicAreaTargetSystem(double range, double coneAngle, int numRays, int numSteps, ICollisionDetector *collisionDetector, ITargetValidator *targetValidator)
{
	this->range = range;
	...
	this->predicate = new ConicAreaGunPredicate;
	this->intersection = new RayIntersection;
	this->distanceByTarget = new QHash<Body*, double>;
}

ConicAreaTargetSystem::~ConicAreaTargetSystem()
{
...
void ConicAreaTargetSystem::getTargets(Body *shooter, double barrelLength, double fireOriginOffsetCoeff, Vector3 *barrelOrigin, Vector3 *gunDir, Vector3 *gunRotationAxis, QVector<Body*> *targetBodies, QVector<double> *targetDistances)
{
	...
	if (this->collisionDetector->intersectRay(barrelOrigin, gunDir, COLLISION_GROUP, barrelLength, this->predicate, this->intersection))
...
void ConicAreaTargetSystem::processSector(Vector3 *rayOrigin, Vector3 *gunDir, Vector3 *rotationAxis, int numRays, double angleStep)
{
	double rayAngle = 0;
...
void ConicAreaTargetSystem::processRay(Vector3 *rayOrigin, Vector3 *rayDirection, double rayLength)
{
	Vector3 origin;
	...
	while (rayLength > 0.1)
	{
		if (this->collisionDetector->intersectRay(&origin, rayDirection, COLLISION_GROUP, rayLength, this->predicate, this->intersection))
		{
