...
class CommonGunPredicate : public IRayCollisionPredicate
{
public:
	Body* shooter;
...
CommonTargetSystem::CommonTargetSystem(double maxDistance, double maxAngleUp, int numRaysUp, double maxAngleDown, int numRaysDown, ICollisionDetector *collisionDetector, ICommonTargetEvaluator *targetEvaluator)
{
	...
	this->predicate = new CommonGunPredicate;
	this->intersection = new RayIntersection;
	this->bestDirection = new Vector3;
}

CommonTargetSystem::~CommonTargetSystem()
{
...
bool CommonTargetSystem::getTarget(Vector3 *barrelOrigin, Vector3 *gunDirection, Vector3 *gunRotationAxis, Body *shooterBody, HitInfo *hitInfo)
{
	this->maxPriority = 0;
	...
	if (this->collisionDetector->intersectRay(barrelOrigin, gunDirection, COLLISION_GROUP, this->maxDistance, predicate, intersection))
	{
...
void CommonTargetSystem::checkSector(Vector3 *barrelOrigin, Vector3 *gunDirection, Vector3 *barrelRotationAxis, int raysNum, double angleStep)
{
	Matrix3 matrix;
